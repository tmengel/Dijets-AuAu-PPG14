...
void Process_Calo_Calib()
{
  ...
  if (Enable::DISABLE_SKIMMER)
  {
    std::cout << "CaloStatusSkimmer is disabled, all events will be processed." << std::endl;
    isSim = true; // this will skip the skimmer, which is the only thing that depends on sim vs data, so we can just set this to true to skip the skimmer if desired
  }
